...
const opcijePogodioZiviZid = [
  {value:0, label:'igrač nije pogodio živi zid'},
  {value:1, label:'igrač je pogodio živi zid'}
]
...
class DodajSlobodanUdarac extends Component {
  constructor(props) {
    super(props);
...
this.setState({errorMsg: 'Error retrieving data'})
  ...
  handleSelectPogodioZiviZidChange = (selectedOption) => {
    this.setState({pogodiozivizid: selectedOption.value});
  }
...
toggleGostujuciIgracGolman= () =>{
  ...
  handleSubmit = e => {
    ...
    if(this.state.brojigracazivizid < 0 || this.state.brojigracazivizid > 10){
        this.setState({errMsg: 'Broj igrača u živom zidu mora biti između 0 i 10'});
        return;
     }
    ...
    if(this.state.pogodiozivizid === null){
      this.setState({errMsg: 'Morate odrediti je li igrač pogodio živi zid'});
      return;
    }
    ...
    udarac.stranaigracslobodni = this.state.stranaigrac;
    udarac.stranagolmanslobodni = this.state.stranagolman;
    udarac.pogodiozivizid = this.state.pogodiozivizid;
    udarac.brojigracazivizid = this.state.brojigracazivizid;
    udarac.udaljenostslobodni = this.state.udaljenost;

    console.log(udarac)

    axios
      .post('http://localhost:3001/admin/dodajslobodni/', udarac)
      .then(() => {
  ...
  render() {
    return (
      <div>
        <Button onClick={this.toggleVisibility}> Dodaj slobodan udarac</Button>
        {this.state.visible && <div className="container">
            <h2>Dodaj slobodan udarac</h2>
          <Form onSubmit={this.handleSubmit}>
            ...
            <br/>
            <div>
              broj igrača u zivom zidu:
              <br/>        
              <input
                type="number"
                name="brojigracazivizid"
                placeholder="broj igrača u živom zidu"
                onChange={this.handleInputChange}
                required
              />
              m
            ...
            <br/>
            <div>
            Je li igrač pogodio živi zid:
            <br/>
            <Select onChange={this.handleSelectPogodioZiviZidChange}
              options={opcijePogodioZiviZid}/>
            </div>
            <br/>
            <div>
...
export default DodajSlobodanUdarac;